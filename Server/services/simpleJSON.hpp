...
class JSON
{
    ...
    public:
        ...
        map<string, JSON> getJsonAsMap(){
            map<string, JSON> mapCopy;
            map<string, JSON>::iterator itr;
            for(itr = Internal.Map->begin(); itr != Internal.Map->end(); itr++){
                mapCopy.insert({itr->first, itr->second});
            }
            return mapCopy;
        }
...
namespace {
    ...
    JSON parse_object( const string &str, size_t &offset ) {
        ...
        while( true ) {
            ...
            consume_ws( str, offset );
            if( str[offset] != ':' ) {
                throw "Error: Object: Expected colon, found '"+std::to_string(str[offset]);
                break;
            }
            consume_ws( str, ++offset );
            ...
            if( str[offset] == ',' ) {
                ...
                throw "ERROR: Object: Expected comma, found '"+std::to_string(str[offset]);
                break;
            }
        }
    ...
    JSON parse_array( const string &str, size_t &offset ) {
        ...
        while( true ) {
            ...
            if( str[offset] == ',' ) {
                ...
                throw "ERROR: Array: Expected ',' or ']', found '"+std::to_string(str[offset]);
                return std::move( JSON::Make( JSON::Class::Array ) );
            }
        }
                ...
                case 'u' : {
                    ...
                    for( unsigned i = 1; i <= 4; ++i ) {
                        ...
                        if( (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F') )
                            ...
                            throw "ERROR: String: Expected hex character in unicode escape, found '"+std::to_string(c);
        ...
        if( c == 'E' || c == 'e' ) {
            ...
            while( true ) {
                ...
                if( c >= '0' && c <= '9' )
                    ...
                    throw "ERROR: Number: Expected a number for exponent, found '"+std::to_string(c);
            ...
            throw "ERROR: Number: unexpected character '"+std::to_string(c);
            return std::move( JSON::Make( JSON::Class::Null ) );
        }
        --offset;
    ...
    JSON parse_bool( const string &str, size_t &offset ) {
        ...
        if( str.substr( offset, 4 ) == "true" )
            ...
            throw "ERROR: Bool: Expected 'true' or 'false', found '"+str.substr( offset, 5 );
            return std::move( JSON::Make( JSON::Class::Null ) );
        }
    ...
    JSON parse_null( const string &str, size_t &offset ) {
        ...
        if( str.substr( offset, 4 ) != "null" ) {
            throw "ERROR: Null: Expected 'null', found '"+str.substr( offset, 4 );
            return std::move( JSON::Make( JSON::Class::Null ) );
        }
        offset += 4;
    ...
    JSON parse_next( const string &str, size_t &offset ) {
        ...
        throw "ERROR: Parse: Unknown starting character '"+std::to_string(value);
        return JSON();
    }
}
